...
import { getEachRepoData } from '@/services/fetchData'
...
onMounted(async () => {
  ...
  try {
    const {
      ...
      languages: langs,
      commits
    } = await getEachRepoData(repoName.value)
...
<template>
  ...
  >
    <div class="w-full max-w-xl bg-white shadow-lg rounded-lg p-6 text-left">
      ...
      <p class="text-sm text-gray-500 mb-2">Default Branch: {{ repositoryData.default_branch }}</p>
      ...
      <div class="flex flex-wrap justify-between mb-3">
        ...
        <p class="text-sm text-gray-500 mb-2 md:mb-0">Forks: {{ repositoryData.forks_count }}</p>
        <p class="text-sm text-gray-500 mb-2 md:mb-0">
